...
function renderStoreList(container, template, collection, type){
    ...
    $.each( collection , function( key, val ) {
        if (type == "stores" || type == "category_stores"){
            if(!val.store_front_url ||  val.store_front_url == null || val.store_front_url.length === 0){
...
function renderHours(container, template, collection, type){
    ...
    if (type == "reg_hours") {
        $.each( collection , function( key, val ) {
            if (val.store_ids == null && val.is_holiday == false) {
    ...
    if (type == "holiday_hours") {
        $.each( collection , function( key, val ) {
            if (val.store_ids == null && val.is_holiday == true) {
...
function renderJobDetails(container, template, collection){
    ...
    $.each( item_list , function( key, val ) {
        if (val.jobable_type == "Store") {
            ...
            if (store_details.store_front_url_abs == null){
